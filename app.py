...
class App(CTk):
    def __init__(self):
        ...
        self.title("LogiTalk")
        """MENU"""
        self.menu_frame = CTkFrame(self, fg_color="#8abbff", width=30, height=400)
        ...
        """SCROLL FRAME"""
        self.scroll_frame = CTkScrollableFrame(self, fg_color="#dcff96")
    ...
    def toggle_menu(self):
        if self.is_menu:
            ...
            self.btn_menu.configure(text="<")
            self.show_menu()
            self.name_lbl = CTkLabel(self.menu_frame, text="Введіть імʼя", font=("Helvetica", 20, "bold"))
            self.name_entry = CTkEntry(self.menu_frame, width=140, placeholder_text="Введіть імʼя")
    ...
    def adaptive_ui(self):
        ...
        self.scroll_frame.configure(width=self.winfo_width() - self.menu_frame.winfo_width(),
                                    height=self.winfo_height() - 70)
        self.msg_text_entry.place(x=self.menu_frame.winfo_width() + 5, y=self.winfo_height() - 50)
        self.msg_text_entry.configure(width=self.winfo_width() - self.menu_frame.winfo_width() -
                                            self.btn_send.winfo_width() - 15)
        self.btn_send.place(x=self.msg_text_entry.winfo_width() + self.menu_frame.winfo_width() + 10,
